...
#define PRIM(TYPE, REP, EQTEST, LTTEST, NULLTEST) struct TYPE {       \
  using rep = REP;                                                    \
  TYPE() = default;                                                   \
...
PRIM_EQ_LT(J, int64_t, x == std::numeric_limits<rep>::min())
...
PRIM_EQ_LT(I, int32_t, x == std::numeric_limits<rep>::min())
